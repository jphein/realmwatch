...
import { updateUI, updateCoreSublabels, updateHASublabels, updateTooltips, updateInfraNodes,
         applyMasterScale, getLastStatus, findStatusKey, findCollectd, setPostUpdateHook,
         getSliderRefs, setMasterScale, getMasterScale,
         getBubbleScale, getBubbleFixedSize, setBubbleFixedSize } from './node-status.js';
import { spawnMote, clearMoteCanvas, updateSparkleRect, ensureMoteLoop, getFpsEl,
...
initTopoControls({ saveSettings, scheduleSave, applyTopoZ: _applyTopoZ });
...
(function initSSE() {
  ...
  sse.addEventListener('status', e => {
    ...
    if (_zoomActive && liveOk) { _deferredStatus = d; if (d.wifi) setWifiMap(d.wifi); return; }
    updateUI(d);
