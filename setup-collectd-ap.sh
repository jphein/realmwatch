...
#
# Requires: SSH key auth to all APs (see scripts/setup-ssl-certs.sh)
# Note:     Uses key-based SSH auth (keys deployed to /etc/dropbear/authorized_keys).
...
deploy_ap() {
    local AP_IP="$1"
    local AP_HOST
    AP_HOST=$(ssh $SSH_OPTS "root@$AP_IP" "cat /proc/sys/kernel/hostname" 2>/dev/null)
    ...
    ssh $SSH_OPTS "root@$AP_IP" "mkdir -p /usr/local/bin"
    cat "$SCRIPT_DIR/collectd-wifi-clients.sh" | ssh $SSH_OPTS "root@$AP_IP" "cat > /usr/local/bin/collectd-wifi-clients.sh && chmod +x /usr/local/bin/collectd-wifi-clients.sh"
...
EXECCONF
    ...
    # Restart collectd
    ssh $SSH_OPTS "root@$AP_IP" "/etc/init.d/collectd restart 2>/dev/null"
