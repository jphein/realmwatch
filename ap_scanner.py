...
_last_scan = {"ts": 0, "ap_clients": {}, "leases": {}}
...
def scan_and_update():
    ...
    # Build MAC→AP map (if a MAC appears on multiple APs, last wins — shouldn't happen)
    mac_to_ap = {}
    for ap_id, macs in ap_clients.items():
        for mac in macs:
            mac_to_ap[mac] = ap_id

    # Build IP→node_id from topology
    ip_to_node = {}
    for n in topo["nodes"]:
        ip = n.get("ip", "")
        if not ip:
    ...
    return {"scanned": len(ap_nodes), "leases": len(leases), "changes": changes}
