...
PALACE_ENV = os.path.expanduser("~/.config/palace-daemon/env")


def _load_dsn() -> str:
    dsn = os.environ.get("PALACE_DB_DSN", "").strip()
    if dsn:
        return dsn
    try:
        with open(PALACE_ENV) as f:
            for line in f:
                line = line.strip()
                if line.startswith("PALACE_DB_DSN="):
                    return line.split("=", 1)[1].strip().strip('"').strip("'")
    except OSError:
        pass
    sys.exit(
        "backfill-poll: PALACE_DB_DSN unset and not present in "
        f"{PALACE_ENV}. Set the env var or add PALACE_DB_DSN=... to that file."
    )
