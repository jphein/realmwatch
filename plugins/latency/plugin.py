"""Latency plugin — batch-pings wired node IPs via fping.

Ported from latency_prober.py into the plugin system. Runs fping against
all wired topology nodes every 30 seconds and produces a {node_id: rtt_ms}
map consumed via SSE and the /latency endpoint.
...
def _probe_once():
    """Single probe cycle: reload topology + run fping."""
...
def _probe_loop():
    """Background thread: probe every PROBE_INTERVAL seconds."""
    while _running:
        try:
            _probe_once()
        except Exception as e:
            print(f"latency plugin: probe error: {e}", flush=True)
...
def _get_latency_sse_data():
    """SSE source getter — returns pre-grouped latency data."""
    import realm_db
    topo_nodes = realm_db.get_topology().get("nodes", [])
    return get_latency_grouped(topo_nodes)


def handle_latency(req, params):
    """GET /latency endpoint handler — returns raw latency map."""
    req.respond(get_latency_map())


def setup(ctx):
    """Plugin setup — register SSE source, endpoint, start probe thread."""
    global _running

    # Register SSE source (latency data, 30s interval, included in burst)
    ctx.register_sse_source(
        event_type="latency",
        getter_fn=_get_latency_sse_data,
        interval=30,
        burst=True,
        burst_priority=4,
    )
    ...
    _running = True
    ctx.start_background_thread(target=_probe_loop, name="plugin-latency-prober")

    # Expose public API for other plugins (e.g., estimate_latency for topology)
    ctx.expose_api({
        "get_latency_map": get_latency_map,
        "get_latency_grouped": get_latency_grouped,
        "get_last_probe_ts": get_last_probe_ts,
        "estimate_latency": estimate_latency,
        "get_subnet": get_subnet,
        "set_wifi_nodes": set_wifi_nodes,
    })

    ctx.log("Arcane Pulse latency prober started (interval=%ds)", _PROBE_INTERVAL)
